resource "google_container_cluster" "standard-cluster-1" {
  ...
  zone               = "${var.zone}"
  initial_node_count = "${var.initial_workers_count}"
  ...
  node_config {
    machine_type = "${var.initial_workers_mach_type}"
    disk_size_gb = "${var.initial_workers_disk_size}"
    oauth_scopes = [
      "compute-rw",
      "storage-ro",
      "logging-write",
...
resource "google_container_node_pool" "bigpool" {
  ...
  node_count = "${var.workers_count_pool}"

  node_config {
    machine_type = "${var.workers_mach_type_pool}"
    disk_size_gb = "${var.workers_disk_size_pool}"
    oauth_scopes = [
      "compute-rw",
      "storage-ro",
      "logging-write",
